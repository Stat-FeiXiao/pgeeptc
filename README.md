...
GEEex_fit <- pgeeptc(formula=Surv(time,event)~x1+x2+x3+x4+x5+x6+x7+x8+x9+x10,id=Data$id, 
              ...
              nopindex=NULL, boots=T, nboot=100, QIC =T, nlambda=100, lambda.min.ratio=1e-4,eps = 1e-5, maxiter = 100, tol = 1e-3)
...
PGEEind_fit <- pgeeptc(formula=Surv(time,event)~x1+x2+x3+x4+x5+x6+x7+x8+x9+x10,id=Data$id,
                       ...
                       nopindex=NULL, boots=FALSE, nboot=100, QIC =T, nlambda=100, lambda.min.ratio=1e-4,eps = 1e-5, maxiter = 100, tol = 1e-3)
