...
#'
#' @description We select the tuning parameters based on the EBIC and fit the PTC model using SCAD-based penalized estimating equations.
#'
...
fitfun <- function(Time, Status, id, Var, X, corstr , stad, beta_int, nopindex=NULL, nlambda, lambda.min.ratio, eps, lambda=NULL, maxiter, tol, QIC){
  ...
  if(is.null(lambda)){
    ...
    bet.init <- c(ifelse(cure.rate==0,0,log(-log(cure.rate))),rep(0,p-1))   #reltol=1e-6
    Bthat <- optim(par=bet.init,fn=Loss, control=list(maxit=500,fnscale=-1,reltol=1e-3), Time=Time, Status= Status, X=X)$par
  ...
  for(s in 1:nlambda){
    ...
    beta1 <- fit_PGEE$beta
    lam1 <- fit_PGEE$lam
    ebic <- EBIC(X,Status,Time, id,beta1,stad,tol)
  ...
  for(ss in 1:length(beta.minEBIC) ){
  if(abs(beta.minEBIC[ss]) < tol) beta.minEBIC[ss] <- 0
  }
  ...
  if(Var){
    ...
    U.H.E.M.val=U_H_E_M(Time,Status,id,X,beta2,fit$Qhat,fit$pphi,fit$lam.minEBIC,nopindex)
    H<-U.H.E.M.val$H
    E<-U.H.E.M.val$E
    M<-U.H.E.M.val$M
    ...
    M1 <- M[beta.minEBIC != 0 ,beta.minEBIC != 0]
    H1 <- H[beta.minEBIC != 0 ,beta.minEBIC != 0]
    E1 <- E[beta.minEBIC != 0 ,beta.minEBIC != 0]

    bet.var <- rep(NA,length(beta.minEBIC)); bet.var[beta.minEBIC!=0] <- diag(MASS::ginv(H1+K*E1)%*%M1%*%t(MASS::ginv(H1+K*E1))  )
    fit$V.b1<- bet.var
  ...
  if( fit$lam.minEBIC == L_grid |  fit$lam.minEBIC ==R_grid ) {
    fit$lam.grad.improper <- TRUE
  }else{
    fit$lam.grad.improper <- FALSE
  }
  if( fit$ lam.minEBIC  == R_grid ) {
    fit$indicateR <- TRUE
  }else{
    fit$indicateR <- FALSE
  }
