...
pgeeptc<-function(formula, id, data, corstr = "independence", stad=TRUE, Ibeta=NULL, Var=FALSE, lambda=NULL, nopindex=NULL, boots=FALSE, nboot=100, QIC =FALSE, nlambda=100, lambda.min.ratio=1e-4, eps = 1e-5, maxiter = 100, tol = 1e-3){
  call <- match.call()
  ...
  if(is.null(Ibeta)) {
    cvfit <- glmnet::cv.glmnet(X[,-1],  Surv(Time,Status) , family = "cox", alpha = 1, cox.ties = "breslow")
  ...
  if(boots){
    ...
    boots_sd<-boots( formula,data,  id, nboot, corstr, Ibeta , nopindex,  fitf$lam.minEBIC, eps, maxiter,stad)
...
print.pgeeptc <- function(x, ...) {
  ...
  if (x$Var) {
    bt <- array(c(x$beta.minEBIC), c(length(x$beta.minEBIC), 4))
