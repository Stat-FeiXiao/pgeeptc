...
boots<-function(formula, data,  id, nboot, corstr, beta_int, nopindex, lam, eps, maxiter,stad){
  Bootsample <- nboot           
  beta_int0 <- beta_int
  K <- length(unique(id))
  BMs <- matrix(0, Bootsample, length(beta_int0))
  ...
  for (rrn in 1:Bootsample) {
    repeat{
      ...
      if(stad){
        Ibeta1 <- rep(0,length(beta_int0))
        cumIbe <- 0
        for(lln in 2:length(beta_int0)){
          cumIbe <- cumIbe + mean(X_boot[,lln]) * beta_int0[lln]
          Ibeta1[lln] =  sd(X_boot[,lln ]) * beta_int0[lln]
        }
        Ibeta1[1] <- beta_int0[1] + cumIbe
        beta_int <- Ibeta1
        
        for (i in 2:ncol(X_boot)) {
          stadX_boot[, i] <- (X_boot[, i] - mean(X_boot[, i]))/sd(X_boot[, i])
        }
      }
      ...
      Status_boot <- Y_boot[, 2]
          tryboot <- try(
            R.utils::withTimeout(
           pgee(Time_boot,Status_boot, id_boot,stadX_boot,corstr ,beta_int,nopindex, lam, eps, maxiter)
    ...
    BMs[rrn, ] <- beta2 
    R.fi.hat=mycor(Status_boot,id_boot,stadX_boot,beta2,Time_boot,corstr)
